namespace NHapi.Base.Parser
{
    ...
    public class DefaultXMLParser : XMLParser
    {
        ...
        public override System.Xml.XmlDocument EncodeDocument(IMessage source)
        {
            ...
            this.Encode(source, doc.DocumentElement);
            return doc;
        }
        ...
        public override IMessage ParseDocument(System.Xml.XmlDocument XMLMessage, System.String version)
        {
            ...
            IMessage message = this.InstantiateMessage(messageName, version, true);
            this.Parse(message, XMLMessage.DocumentElement);
            return message;
        }
            ...
            try
            {
                for (int i = 0; i < childNames.Length; i++)
                {
                    ...
                    for (int j = 0; j < reps.Length; j++)
                    {
                        ...
                        {
                            hasValue = true;
                            this.Encode((IGroup)reps[j], childElement);
                        }
                        else if (reps[j] is ISegment)
                        {
                            hasValue = this.Encode((ISegment)reps[j], childElement);
                        }
            ...
            System.String theName)
        {
            ...
            for (int i = 0; i < children.Count; i++)
            {
                ...
                if (System.Convert.ToInt16(child.NodeType) == (short)System.Xml.XmlNodeType.Element
                    && child.Name.Equals(theName))
                {
                    result.Add(child);
                }
            }
        ...
        private void Parse(IGroup groupObject, System.Xml.XmlElement groupElement)
        {
            ...
            for (int i = 0; i < allChildNodes.Count; i++)
            {
                ...
                if (System.Convert.ToInt16(node.NodeType) == (short)System.Xml.XmlNodeType.Element
                    && !unparsedElementList.Contains(name))
                {
        ...
        private void ParseReps(
            ...
            System.String childIndexName)
        {
            System.Collections.IList reps = this.GetChildElementsByTagName(
                groupElement,
                MakeGroupElementName(messageName, childName));
            ...
            if (groupObject.IsRepeating(childIndexName))
            {
                ...
                {
                    this.ParseRep((System.Xml.XmlElement)reps[i], groupObject.GetStructure(childIndexName, i));
                }
            }
            else
            {
                if (reps.Count > 0)
                {
                    this.ParseRep((System.Xml.XmlElement)reps[0], groupObject.GetStructure(childIndexName, 0));
                }
                ...
                if (reps.Count > 1)
                {
                    ...
                    {
                        this.ParseRep((System.Xml.XmlElement)reps[i], groupObject.GetStructure(newIndexName, i - 1));
                    }
                }
            }
        }
